...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect {Dessert.new(1)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      ...
      expect(brownie.add_ingredients).to include("milk")
    end
  end
 ...
 it "raises an error if the amount is greater than the quantity" do
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Amazing")
      expect(brownie.serve).to eq("Chef Amazing has made 100 brownies!")
    end
  end
